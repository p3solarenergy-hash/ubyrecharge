...
(function () {
  ...
  function investidores(inv) {
    return `
      ...
      <section class="section"><div class="section-head"><div><p class="kicker">Cotistas</p><h2>Apuração por cotista</h2><p>Investimento de referência: R$ 80.000 por cota. Payback indicativo pelo retorno anualizado.</p></div></div>
        <div class="table-wrap"><table><thead><tr><th>Cotista</th><th class="num">Cotas</th><th>Início</th>${inv.months.map(m => `<th class="num">${esc(m.label)}</th>`).join("")}<th class="num">Total devido</th><th class="num">Retorno acum.</th><th class="num">Payback</th><th>Situação</th></tr></thead>
          <tbody>${inv.investors.map(i => `<tr><td><strong>${esc(i.name)}</strong></td><td class="num">${i.quotas}</td><td>${esc(i.eligibleFrom ? UBY.state.api.monthName(i.eligibleFrom) : "—")}</td>${i.allocations.map(a => `<td class="num">${fmt.brl(a)}</td>`).join("")}
            <td class="num"><strong>${fmt.brl(i.due)}</strong></td><td class="num">${fmt.pct(i.returnRate * 100)}</td><td class="num">${i.paybackYears ? `${fmt.n1(i.paybackYears)} anos` : "—"}</td><td>${esc(i.status)}</td></tr>`).join("") || `<tr><td colspan="${7 + inv.months.length}" class="empty">Nenhum cotista cadastrado.</td></tr>`}</tbody></table></div>
      </section>`;
  }
