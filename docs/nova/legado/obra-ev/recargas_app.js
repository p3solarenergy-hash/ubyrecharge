...
function scheduledPaymentApplies(item, monthKeyValue) {
  if (!item?.enabled || !/^\d{4}-\d{2}$/.test(monthKeyValue || '')) return false;
  if (item.scheduledPayment) return (!item.startMonth || item.startMonth <= monthKeyValue) && (!item.endMonth || item.endMonth >= monthKeyValue);
  return matrizCashAmount(item, monthKeyValue) > 0;
}
...
function scheduledPaymentTarget(item) {
  ...
  return { workId: String(target.workId || String(target.scope || '').split('::')[0] || ''), station: target.station || (item?.scheduledPayment ? '' : 'Matriz UBY'), workName: target.workName || '', targetCount: targets.length };
}
...
function renderScheduledPayments(unitData = getGeneralUnitData()) {
  ...
  const totals = payments.reduce((sum, item) => {
    const status = scheduledPaymentStatus(item, selectedMonth), amount = item.scheduledPayment ? Number(item.amount || 0) : matrizCashAmount(item, selectedMonth);
  ...
  host.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:14px;flex-wrap:wrap"><div><h2 style="margin:0">Pagamentos programados por carregador</h2><p class="sub" style="margin:6px 0 0;max-width:78ch">Calendário central de caixa: acompanha os pagamentos por carregador e também os custos da matriz já cadastrados, sem duplicar valores nem alterar o DRE.</p></div><label class="sub">COMPETÊNCIA<input id="scheduledPaymentMonth" type="month" value="${escapeAttr(selectedMonth)}" onchange="renderScheduledPayments(getGeneralUnitData())" style="display:block;margin-top:5px;background:var(--p3-card-soft);border:1px solid var(--p3-border);color:var(--p3-text);border-radius:8px;padding:9px 12px;font:inherit"></label></div>
    ...
    <div style="overflow-x:auto;margin-top:16px"><table><thead><tr><th>Pagamento</th><th>Carregador / obra</th><th>Vencimento</th><th>Valor</th><th>Situação</th><th style="text-align:right">Ação</th></tr></thead><tbody>${payments.length ? payments.map(item => { const target = scheduledPaymentTarget(item), status = scheduledPaymentStatus(item, selectedMonth), due = scheduledPaymentDueDate(item, selectedMonth).toLocaleDateString('pt-BR'), amount = item.scheduledPayment ? Number(item.amount || 0) : matrizCashAmount(item, selectedMonth), source = item.scheduledPayment ? '' : 'Custo da matriz'; return `<tr><td><strong>${escapeHtml(item.name)}</strong><div class="sub">${source ? `${source} | ` : ''}${escapeHtml(item.category)}${item.supplier ? ` | ${escapeHtml(item.supplier)}` : ''}</div></td><td>${escapeHtml(target.station || 'Carregador não identificado')}<div class="sub">${escapeHtml(target.workName || (target.targetCount > 1 ? `rateado para ${target.targetCount} carregadores` : ''))}</div></td><td>${due}<div class="sub">todo dia ${item.dueDay}</div></td><td class="num" style="text-align:right">${fmtBRL(amount)}</td><td><span class="finance-setting-state ${status.className}">${status.label}</span></td><td style="text-align:right">${status.key === 'paid' ? `<button class="btn-recalc" type="button" onclick="setScheduledPaymentStatus('${escapeAttr(item.id)}','${escapeAttr(selectedMonth)}','pending')">Reabrir</button>` : `<button class="btn-open" type="button" onclick="setScheduledPaymentStatus('${escapeAttr(item.id)}','${escapeAttr(selectedMonth)}','paid')">Marcar pago</button>`}${item.scheduledPayment ? ` <button class="btn-recalc" type="button" onclick="openFinanceUnit('${escapeAttr(target.workId)}','${escapeAttr(target.station)}')">Abrir</button>` : ''}</td></tr>`; }).join('') : '<tr><td colspan="6" style="text-align:center;color:var(--p3-muted);padding:18px">Nenhum pagamento programado nesta competência.</td></tr>'}</tbody></table></div>
...
function setScheduledPaymentStatus(id, monthKeyValue, status) {
  const item = loadMatrizCosts().find(cost => cost.id === id); if (!item || !/^\d{4}-\d{2}$/.test(monthKeyValue || '')) return;
...
function rechargeSourcePlatform(charge = {}) {
  ...
  const fromFile = safeText(importedFile?.sourcePlatform).trim();
  if (fromFile) return fromFile;
  // NOVA PLATAFORMA (25/09/2026): sessões lidas do Supabase não trazem a
  // plataforma e loadedFiles fica vazio, o que zerava a regra Spott. O layout
  // Spott grava a coluna "Aplicativo" em paymentType ("Spott") e os arquivos
  // exportados pelo Spott se chamam "lista-de-transacoes".
  if (normalizeHeaderName(charge.paymentType) === 'spott' || /lista-de-transacoes/i.test(fileName)) return 'Spott';
  return '';
}
...
function generateNetworkUnifiedReport() {
  ...
  const occupancyCards = model.rows.map(row => {
    ...
    const kind = ['ac', 'dc'].includes(String(row.kind || '').toLowerCase())
      ? String(row.kind).toLowerCase()
      : chargerKind({ station: row.stationName || row.station, workName: row.workName, connType: row.connType });
    return {
      ...
      hours: Number(occupancy.hours || 0), operationStart: occupancy.operationStart, partner, kind
    };
  }).sort((a, b) => b.rawPct - a.rawPct || a.name.localeCompare(b.name, 'pt-BR'));
  const occupancyGroups = [
    { key: 'uby-dc', title: 'Carregadores DC', detail: 'Ativos próprios UBY · métricas principais de recarga rápida', partner: false, kind: 'dc' },
    { key: 'uby-ac', title: 'Carregadores AC', detail: 'Ativos próprios UBY · acompanhamento separado das médias DC', partner: false, kind: 'ac' },
    { key: 'partner-dc', title: 'Parceiros DC · royalties', detail: 'Leitura operacional · receita UBY reconhecida somente como royalties', partner: true, kind: 'dc' },
    { key: 'partner-ac', title: 'Parceiros AC · royalties', detail: 'Leitura operacional · receita UBY reconhecida somente como royalties', partner: true, kind: 'ac' },
    { key: 'other', title: 'Carregadores sem classificação AC/DC', detail: 'Cadastre o tipo do conector para entrar no grupo correto.', partner: null, kind: 'unknown' }
  ].map(group => ({
    ...group,
    cards: occupancyCards.filter(card => group.key === 'other'
      ? !['ac', 'dc'].includes(card.kind)
      : card.partner === group.partner && card.kind === group.kind)
  })).filter(group => group.cards.length);
  const occupancyCardMarkup = card => `<article class="occupancy-card"><div class="occupancy-top"><div class="pie" style="--pct:${card.pct.toFixed(2)}"><b>${fmtPct(card.rawPct)}</b></div><div><div class="occupancy-name">${escapeHtml(card.name)}</div><small>${escapeHtml(card.workName)}</small></div></div><div class="occupancy-meta">${fmtKWh(card.energy)} entregues de ${fmtKWh(card.maxKWh)} disponíveis · ${card.hours.toFixed(0)} h${card.operationStart ? ` · operação desde ${escapeHtml(fmtDateOnly(card.operationStart))}` : ''}</div><div class="occupancy-bar"><span style="width:${card.pct.toFixed(2)}%"></span></div></article>`;
  ...
  <section><h2>Ocupação da rede por carregador</h2><div class="sub">Energia entregue em relação à capacidade disponível no período. O denominador começa na data real de entrada em operação; se ela ainda não foi cadastrada, usa a primeira recarga importada como referência provisória. DC, AC e parceiros são separados para não misturar métricas operacionais com royalties.</div>${occupancyGroups.map(group => `<div class="occupancy-group ${group.partner ? 'partner' : ''} ${group.kind === 'ac' ? 'ac' : ''}"><div class="occupancy-group-head"><h3>${escapeHtml(group.title)} · ${group.cards.length}</h3><span>${escapeHtml(group.detail)}</span></div><div class="occupancy-grid">${group.cards.map(occupancyCardMarkup).join('')}</div></div>`).join('') || '<div class="sub">Sem base de ocupação disponível.</div>'}</section>
