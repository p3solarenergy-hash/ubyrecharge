(function () {
  ...
  function client() {
    ...
    if (!window.UBY_SUPABASE_CLIENT) {
      const raw = window.supabase.createClient(config.url, config.anonKey);
  ...
  // NOVA PLATAFORMA (aprovado pelo usuário em 26/09/2026): a gravação só é
  // liberada na tela "Importar planilhas", e só para as operações de
  // importação de recargas e do cadastro de clientes (importar, desfazer,
  // corrigir mês, remover arquivo, restaurar backup). Configurações, obras,
  // financeiro, tarefas, documentos e os motores seguem bloqueados.
  const IMPORT_SCOPE = {
    name: "importacao",
  ...
  function hideNonImportTabs() {
    const style = document.createElement("style");
    // Esconde toda aba que não seja Detalhes (sessões/importação) ou Clientes,
    // inclusive Mensal/Financeiro da estação, que também gravam (fechamentos).
    style.textContent = "[onclick*=\"switchTab('\"]:not([onclick*=\"'detalhes'\"]):not([onclick*=\"'clientes'\"]),[onclick*=\"openUbyReports\"]{display:none!important}";
    (document.head || document.documentElement).appendChild(style);
  }

  // NOVA PLATAFORMA: enquanto a validacao local nao terminar, nenhuma escrita
  // chega ao Supabase de producao. Leituras e login continuam normais.
  function readOnlyError(action) {
    const error = new Error(`Modo somente leitura: "${action}" bloqueado na Nova Plataforma local. Use a plataforma publicada para gravar.`);
    error.code = "UBY_READ_ONLY";
    console.warn("[UBY somente leitura]", action);
    return error;
  }

  function blockedChain(action) {
    const result = Promise.resolve({ data: null, error: readOnlyError(action), count: null });
  ...
  function readOnlyClient(raw, scope = null) {
    const writeMethods = new Set(["insert", "upsert", "update", "delete"]);
    const storageWrites = new Set(["upload", "update", "remove", "move", "copy"]);
    const allowed = (table, method) => !!scope && (scope.tables[table] || []).includes(method);
        ...
        if (prop === "from") {
          return table => {
            ...
            return new Proxy(builder, {
              get(b, method) {
                if (writeMethods.has(method) && !allowed(table, method)) return () => blockedChain(`${method} em ${table}`);
                const value = b[method];
                return typeof value === "function" ? value.bind(b) : value;
              }
            });
          };
        }
        if (prop === "rpc") return (name, ...args) => scope && scope.rpcs.includes(name) ? target.rpc(name, ...args) : blockedChain(`rpc ${name}`);
        if (prop === "storage") {
          const storage = target.storage;
          return new Proxy(storage, {
            get(s, method) {
              if (method !== "from") { const v = s[method]; return typeof v === "function" ? v.bind(s) : v; }
              return bucket => {
  ...
  async function loadRechargeHistory(workId, options = {}) {
    const sb = client();
    if (!sb) throw new Error('Supabase ainda nao configurado.');
    if (!(await currentUser())) throw new Error('Entre no Supabase para consultar os backups.');
    const limit = Math.min(100, Math.max(1, Number(options.limit || 30)));
    const { data, error } = await sb
      .from('obra_recargas_historico')
      .select('id,obra_id,acao,origem,arquivos,recargas,resumo,recargas_count,arquivos_count,usuario_email,base_updated_at,created_at')
      .eq('obra_id', String(workId || 'geral'))
      .order('created_at', { ascending: false })
      .limit(limit);
    if (error) throw error;
    return (data || []).map(row => ({
      ...
      action: row.acao,
      origin: row.origem,
      files: Array.isArray(row.arquivos) ? row.arquivos : [],
      charges: Array.isArray(row.recargas) ? row.recargas : [],
      summary: row.resumo || {},
      filesCount: Number(row.arquivos_count ?? (Array.isArray(row.arquivos) ? row.arquivos.length : 0)),
      chargesCount: Number(row.recargas_count ?? (Array.isArray(row.recargas) ? row.recargas.length : 0)),
  ...
  async function restoreRechargeHistorySnapshot(workId, snapshotId) {
    const sb = client();
    if (!sb) throw new Error('Supabase ainda nao configurado.');
    if (!(await currentUser())) throw new Error('Entre no Supabase antes de restaurar um backup.');
    const { data, error } = await sb.rpc('restore_recharge_snapshot_atomic', {
      p_obra_id: String(workId || 'geral'),
      p_snapshot_id: String(snapshotId || '')
    });
    if (error) throw error;
    return data || {};
  }
  ...
  window.UBY_SUPABASE = {
    ...
    loadRechargeBase,
    loadRechargeHistory,
    restoreRechargeHistorySnapshot,
