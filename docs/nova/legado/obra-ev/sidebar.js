(function () {
  ...
  if (!profile && currentModule !== "login") {
    if (embedded) { window.top.location.href = new URL("../../login.html", location.href).href; return; }
  ...
  // NOVA PLATAFORMA: dentro da moldura nova, o menu antigo some e a pagina
  // ocupa toda a area de conteudo. Links internos continuam funcionando.
  if (embedded) {
    document.body.classList.add("uby-embedded");
    document.documentElement.classList.add("uby-embedded");
    const embedTheme = document.createElement("link");
    embedTheme.rel = "stylesheet";
    embedTheme.href = base + "embed-theme.css?v=1";
    document.head.appendChild(embedTheme);
    const embedRecolor = document.createElement("script");
    embedRecolor.src = base + "embed-theme.js?v=2";
    document.head.appendChild(embedRecolor);
    const embedStyle = document.createElement("style");
    embedStyle.textContent = "body.uby-embedded .uby-sidebar,body.uby-embedded .uby-topbar{display:none!important}body.uby-embedded.uby-shell{padding-left:0!important}body.uby-embedded.uby-shell .page{padding-top:18px!important}";
