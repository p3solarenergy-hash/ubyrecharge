...
// Ativos que compõem o Painel UBY. Pontos de outras operações permanecem
// preservados no banco, mas não podem entrar no resultado diário da rede.
...
Deno.serve(async () => {
  try {
    ...
    const rows = all.filter(charge => UBY_WORK_IDS.has(s(charge.obra_id))).filter(executed);
